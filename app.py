...
def _new_conn():
    ...
    import urllib.parse
    raw = st.secrets["DATABASE_URL"].strip()
    # psycopg2 needs password URL-encoded if it has special chars
    try:
        result = urllib.parse.urlparse(raw)
        password = urllib.parse.unquote(result.password or "")
        encoded_pw = urllib.parse.quote(password, safe="")
        url = raw.replace(f":{result.password}@", f":{encoded_pw}@", 1)
    except Exception:
        url = raw
...
try:
    ...
    2. **Settings** → **Database** → scroll to **Connection pooling**
    3. Set Mode = **Transaction**, copy the connection string
    4. Replace **[YOUR-PASSWORD]** with your actual password
    5. Go to Streamlit → **Settings** → **Secrets** → paste:
